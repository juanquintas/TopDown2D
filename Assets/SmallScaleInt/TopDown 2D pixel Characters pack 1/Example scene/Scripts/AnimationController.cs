...
namespace SmallScaleInc.TopDownPixelCharactersPack1
{
    public class AnimationController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                bool crouchActive = playerController != null ? playerController.isCrouching : isCrouching;
        ...
        void HandleMovement()
        {
            ...
            {
                moveVector = playerController.CurrentMovementDirection;
                hasMovement = moveVector.sqrMagnitude > 0.0001f;
                isCrouching = playerController.isCrouching;
            }
            else if (rb != null)
            {
                Vector2 velocity = rb.velocity;
                hasMovement = velocity.sqrMagnitude > velocityThreshold * velocityThreshold;
                if (hasMovement)
                {
                    moveVector = velocity.normalized;
                }
            }
            ...
            if (playerController != null)
            {
                facingAngle = playerController.CurrentFacingAngle;
            }
            else
            {
                facingAngle = hasMovement ? Mathf.Atan2(moveVector.y, moveVector.x) * Mathf.Rad2Deg : 0f;
            }
            ...
            isRunning = hasMovement && forwardDot > movementDotThreshold;
            isRunningBackwards = hasMovement && forwardDot < -movementDotThreshold;
            isStrafingLeft = hasMovement && rightDot < -movementDotThreshold;
            isStrafingRight = hasMovement && rightDot > movementDotThreshold;

            isCurrentlyRunning = hasMovement;
            ...
            animator.SetBool("isCrouchRunning", isRunning && isCrouching);
